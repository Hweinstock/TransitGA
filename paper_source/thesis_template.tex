...
\title[Algorithmic Transit Optimization]{Evolutionary Algorithms Applied to Transit System Optimization} % here, the command format is  \title[Abbreviated Title for Header]{Full Title}. But if your title is short, simply use \title{Full Title}
...
\begin{thm}[{\cite[Theorem A]{cg}}]  %Note the extra curly braces, needed to embed the cite command inside the argument to the theorem environment
...
\noindent \includegraphics[width=5in]{3Dgraph}
...
\item See Theorem~\ref{thm:citation in header} \cite[Theorem~A]{cg}; this theorem is typeset with the citation in its descriptive label and in the header of its proof. 
\end{enumerate}
...
Depending on which front-end program you are using, typesetting a document that uses BibTeX can be 
 more complicated than typesetting a plain  \LaTeX\ document. 
First, the good news: BibTeX processing is done automatically, without any extra steps, when you use
\begin{itemize}
\item \href{https://www.overleaf.com/}{Overleaf}, 
%\footnote{An account on Overleaf is free; however, synchronization with Dropbox is a premium feature. If you use the free version and thus don't have an automatic backup, I recommend that you export your project periodically to keep a local copy.},
\item TeXShop, provided you have the appropriate ``magic comment'' in your source file,\footnote{The \emph{very first line} of your source file should be as follows \\ \texttt{\% !TEX program = pdflatexmk}\\ (You'll see it in the source file for this document.)}
\item TeXworks, \textbf{provided} you have a ``magic comment'' as above \textbf{and} you have extended the base configuration of TeXworks appropriately.\footnote{See the instructions for adding \texttt{latexmk} to TeXWorks at \url{https://github.com/TeXworks/texworks/wiki/AdvancedTypesettingTools}.}
\end{itemize}

Otherwise (e.g., if you use the default configuration of TeXworks), you will need several extra steps to use BibTeX; see below.

%Depending on what front-end program you are using, typesetting a \LaTeX\ document that uses biblatex may be 
%more complicated than typesetting a plain  \LaTeX\ document. 
%If you are using the online \LaTeX\ service \href{https://www.overleaf.com/}{Overleaf}\footnote{An account on Overleaf is free; however, synchronization with Dropbox is a premium feature. If you use the free version and thus don't have an automatic backup, I recommend that you export your project periodically to keep a local copy.},
%or if you are using the Mac program TeXShop \textbf{and} the first line of your source file matches the first line of this source file, then BibTeX/biblatex processing is done automatically, without any extra steps. 
%Otherwise (in particular, if you are using MikTex/TeXWorks on Windows), see below for the extra steps. 
...
\subsubsection{Extra steps}
Let me illustrate the extra steps using this source file
 (\texttt{\jobname.tex}), as an example. 
(The footnotes tell you what is happening at each step, but you don't need this detailed information to get started.)

\begin{enumerate} 
\item First, run \LaTeX\ on \texttt{\jobname.tex}, as usual--- you do this  by simply pressing the ``Go" button in your front-end program.\footnote{This step not only 
...
\item Run BibTeX on \texttt{\jobname.tex}; to do so, click on the drop-down menu next to the ``Go" button, choose the ``BibTeX" action, and then click ``Go".\footnote
...
\item Re-run \LaTeX\ on \texttt{\jobname.tex}: switch the drop-down menu back to ``LaTeX"/``pdfLaTeX" and click ``Go".\footnote{Here, the biblatex package typesets the reference list, adding it to the pdf output. Some citations may still be marked as [?], to be adjusted in the next step.}
\item Finally, run  \LaTeX\ on \texttt{\jobname.tex} one more time.\footnote{In this last step, \LaTeX\ inserts correct citation numbers.}
\end{enumerate} 

That's it! The process is only a bit more complicated than typesetting a vanilla \LaTeX\ document. 

\subsection{Troubleshooting}
If you run into trouble, the warnings and errors that appear in the console window may help.
However, if you can't sort out the error messages, the next thing to do is to start afresh by deleting the various auxillary files that are generated when you typeset your source file. 
In particular, you can safely delete
\begin{itemize}
\item the \texttt{.aux} file (as discussed in footnotes 6 and 7), 
\item any \texttt{.bbl} files (as discussed in footnote 7),
\item  all files ending with \texttt{-blx.bib}.
\end{itemize}
You can simply delete these files by hand; alternatively, in TeXShop, you can use the menu: \texttt{File->Trash Aux Files}.
